...
class Socket
{
  public:
    ...
    bool SetBlock()
    {
      ...
      int ret = fcntl(_sock,F_SETFD,fl|O_NONBLOCK);
      if(ret < 0)
      {
        std::cerr << "fcntl error" << std::endl;
        return false;
    ...
    bool SendBlock(const std::string& buff) 
    {
      ssize_t pos = 0; //记录当前位置
      ssize_t left = buff.size();
      for(;;)
      {
        ssize_t write_size = send(_sock,buff.data()+pos,left,0);
        if(write_size < 0)
        {
